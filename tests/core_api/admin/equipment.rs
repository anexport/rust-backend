...
use common::auth0_test_helpers::{create_auth0_token, create_auth0_token_with_email};
use common::fixtures;
use common::TestDb;
...
#[actix_rt::test]
async fn test_admin_action_with_renter_token_but_db_admin_role() {
    let test_db = common::setup_test_db().await;
    let app = setup_app(test_db.pool().clone()).await;
    let user_repo = UserRepositoryImpl::new(test_db.pool().clone());
    let equipment_repo = EquipmentRepositoryImpl::new(test_db.pool().clone());
    ...
    // But token says RENTER
    let token = create_auth0_token_with_email(admin.id, "renter", Some(admin.email.clone()));
    ...
    // Admin action (toggle availability) should succeed because DB role is admin
    let req = actix_test::TestRequest::put()
        .uri(&format!("/api/v1/admin/equipment/{}/availability", eq.id))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .set_json(serde_json::json!({ "is_available": false }))

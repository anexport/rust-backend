...
async fn test_upsert_user_from_auth0_new_user() -> AppResult<()> {
    ...
    assert_eq!(user.avatar_url, Some("https://example.com/photo.jpg".to_string()));

    // Verify identity was created
    let identity = auth_repo.find_identity_by_provider_id("auth0", "auth0|new-user").await?.unwrap();
...
async fn test_upsert_user_from_auth0_existing_user_no_drift() -> AppResult<()> {
    ...
    let auth_repo = Arc::new(MockAuthRepo::default());
    auth_repo.create_identity(&AuthIdentity {
        id: Uuid::new_v4(),
        user_id,
...
async fn test_upsert_user_from_auth0_data_drift_resolution() -> AppResult<()> {
    ...
    assert_eq!(updated_user.avatar_url, Some("https://example.com/new.jpg".to_string()));
...
async fn test_upsert_user_from_auth0_race_condition_fallback() -> AppResult<()> {
    ...
    impl AuthRepository for RaceConditionAuthRepo {
        ...
        async fn find_identity_by_user_id(&self, user_id: Uuid, provider: &str) -> AppResult<Option<AuthIdentity>> {
            self.inner.find_identity_by_user_id(user_id, provider).await
        }
        async fn find_identity_by_provider_id(&self, provider: &str, provider_id: &str) -> AppResult<Option<AuthIdentity>> {
            self.inner.find_identity_by_provider_id(provider, provider_id).await
        }
